...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        left_layout.addWidget(self._build_columns_area(), stretch=1)
        left_layout.addWidget(self._build_action_bar())
        left_layout.addWidget(self._build_status_strip())
        
        main_h_layout.addWidget(left_panel, stretch=6)
        
        # Right panel for 3D View
        right_panel = QWidget()
        right_panel.setStyleSheet("background-color: #1c1d27; border-left: 1px solid #2e2f3e;")
        right_layout = QVBoxLayout(right_panel)
        right_layout.setContentsMargins(12, 12, 12, 12)
        right_layout.setSpacing(10)
        
        title_lbl = QLabel("3D Kinematics Monitor")
        title_lbl.setStyleSheet("font-size: 15px; font-weight:bold; color: #a78bfa; border: none;")
        title_lbl.setAlignment(Qt.AlignCenter)
        right_layout.addWidget(title_lbl)
        
        # Lazy import of 3D widget to avoid slowing down startup if matplotlib is missing
        from robot_3d_widget import Robot3DWidget
        self.robot_view = Robot3DWidget()
        right_layout.addWidget(self.robot_view, stretch=1)
        
        # Inverse Kinematics Controls
        ik_layout = QHBoxLayout()
        ik_layout.addWidget(QLabel("Target X:"))
        self.ik_x = QLineEdit("200")
        self.ik_x.setMaximumWidth(60)
        ik_layout.addWidget(self.ik_x)
        
        ik_layout.addWidget(QLabel("Y:"))
        self.ik_y = QLineEdit("0")
        self.ik_y.setMaximumWidth(60)
        ik_layout.addWidget(self.ik_y)
        
        ik_layout.addWidget(QLabel("Z:"))
        self.ik_z = QLineEdit("200")
        self.ik_z.setMaximumWidth(60)
        ik_layout.addWidget(self.ik_z)
        
        self.ik_btn = QPushButton("Go IK")
        self.ik_btn.setObjectName("ConnectBtn")
        self.ik_btn.clicked.connect(self._on_ik_calculate)
        ik_layout.addWidget(self.ik_btn)
        
        right_layout.addLayout(ik_layout)
        
        self.ee_pos_lbl = QLabel("EE Pos: X=0.0 Y=0.0 Z=0.0")
        self.ee_pos_lbl.setStyleSheet("color:#34d399; font-weight:bold; font-size:13px; border:none;")
        self.ee_pos_lbl.setAlignment(Qt.AlignCenter)
        right_layout.addWidget(self.ee_pos_lbl)
        
        main_h_layout.addWidget(right_panel, stretch=4)
    ...
    def _on_setpoints_changed(self):
        ...
        q = [setpoints.get(i, 0.0) for i in range(4)]
        if hasattr(self, 'robot_view'):
            self.robot_view.update_plot(q)
            # Update EE text
            pos = self.robot_view.robot.get_positions(q)[-1]
            self.ee_pos_lbl.setText(f"EE Pos: X={pos[0]:.1f} Y={pos[1]:.1f} Z={pos[2]:.1f}")
            
        self._worker.set_auto_setpoints(setpoints)

    def _on_ik_calculate(self):
        try:
        ...
        current_q = [float(p.get_setpoint()) for p in self.panels]
        new_q = self.robot_view.robot.inverse_kinematics([x, y, z], current_q)
        
        # Move sliders
        for i, q_val in enumerate(new_q):
            if i < len(self.panels):
                self.panels[i].pos_slider.setValue(int(round(q_val)))
                
        self.robot_view.update_plot(new_q, target_pos=[x, y, z])
